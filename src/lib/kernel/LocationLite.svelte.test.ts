...
describe('LocationLite', () => {
    ...
    describe('getState', () => {
        test.each<{ hash: Hash; expectedState: any }>([
            ...
            `Should return the state associated with the "$hash" hash value.`,
            ({ hash, expectedState }) => {
                // Arrange.
                const testState = {
                    path: 1,
        ...
        test.each([
            ...
            'Should return correct state for undefined hash parameter with $scenario defaultHash.',
            (tc) => {
                // Arrange.
                const testState = {
                    ...
                    hash: {
                        single:
                            tc.defaultHash === ALL_HASHES.single ? tc.expectedStateValue : 'xxx',
                        [typeof tc.defaultHash === 'string' ? tc.defaultHash : '']:
                            tc.defaultHash === 'customHash' ? tc.expectedStateValue : 'xxx'
                    }
                };
                browserMocks.setState(testState);
    ...
    describe('goTo', () => {
        test.each<{
            ...
            'Should $text the query string when instructed by the value $preserveQuery in the preserveQuery option.',
            ({ qs, preserveQuery, expectedQs }) => {
                // Arrange.
    ...
    describe('navigate', () => {
        ...
        test.each<{
            ...
            'Should throw an error when the hash option is $hash and $desc routing is disallowed.',
            ({ hash, options }) => {
                // Arrange.
                const newPath = '/new/path';
