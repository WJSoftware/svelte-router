...
export class RouterEngine {
     ...
     */
    #routePatterns = $derived(
        Object.entries(this.routes).reduce((map, [key, route]) => {
    ...
    constructor(parentOrOpts?: RouterEngine | RouterEngineOptions) {
        if (!location) {
            throw new Error(
                "The routing library hasn't been initialized.  Execute init() before creating routers."
            );
        }
        if (isRouterEngine(parentOrOpts)) {
            ...
            this.#resolvedHash =
                this.#parent && parentOrOpts?.hash === undefined
                    ? this.#parent.resolvedHash
                    : resolveHashValue(parentOrOpts?.hash);
            ...
            if (
                ...
                this.#resolvedHash &&
                typeof this.#resolvedHash !== 'string'
            ) {
                throw new Error(
