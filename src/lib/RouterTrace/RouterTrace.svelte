<script lang="ts">
    ...
    const childRouterRefs = $derived(
        traceOptions.routerHierarchy ? getAllChildRouters(router) : []
    );
...
<table class={[cssClass ?? 'rt-stock', darkTheme && 'dark']} {...restProps}>
    <caption>
        <div>
            <div>
            ...
            >
                {#if router?.parent}
                    Parent: <span class="router-property"
                        >{@render routerId(router.parent, true)}</span
                    >
                {:else}
                    (No parent)
                {/if}
