...
describe('buildHref', () => {
    ...
    describe('Query parameter merging', () => {
        ...
        test('Should handle multiple parameters in both pieces.', () => {
            ...
            expect(result).toBe(
                '/path?param1=value1&param2=value2&param3=value3&param4=value4#hash'
            );
        });
    ...
    describe('preserveQuery option', () => {
        beforeEach(() => {
            ...
            location.url.href =
                'https://example.com/current?preserve1=value1&preserve2=value2&preserve3=value3';
        });
        ...
        test('Should preserve all current query parameters when preserveQuery is true.', () => {
            ...
            expect(result).toBe(
                '/path?new=param&preserve1=value1&preserve2=value2&preserve3=value3#hash'
            );
        });
        ...
        test('Should preserve specific query parameters when preserveQuery is an array.', () => {
            ...
            const result = buildHref(pathPiece, hashPiece, {
                preserveQuery: ['preserve1', 'preserve3']
            });
    ...
    describe('Edge cases', () => {
        ...
        test('Should handle URLs with special characters in parameters.', () => {
            ...
            const hashPiece =
                'https://example.com/any-path?encoded=test%2Bvalue#hash%20with%20spaces';

            const result = buildHref(pathPiece, hashPiece);

            expect(result).toBe(
                '/path?special=hello+world&encoded=test%2Bvalue#hash%20with%20spaces'
            );
        });
    ...
    describe('Cross-universe redirection use case', () => {
        test('Should support typical cross-universe redirection scenario.', () => {
            ...
            const hashUniverseHref =
                'https://example.com/current#/dashboard/settings?hashParam=value';

            const result = buildHref(pathUniverseHref, hashUniverseHref);

            expect(result).toBe(
                '/users/profile?pathParam=value#/dashboard/settings?hashParam=value'
            );
        });
